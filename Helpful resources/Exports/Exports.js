...
export class Exports extends React.Component {
  ...
  async componentDidMount() {
    ...
    const incomingData = await axios.get("http://localhost:5000/api/exports/fields/capture");
  ...
  async refreshFilters() {
    ...
    const incomingFilters = await axios.post("http://localhost:5000/api/exports/filters/get-by-username", refreshFiltersJson);
  ...
  async submitForm() {
    ...
    const resp = await axios.post("http://localhost:5000/api/exports/csv", outgoingJson, {responseType: 'blob'});
  ...
  async saveFilters() {
    const filter_set = {
      ...
      await axios.post("http://localhost:5000/api/exports/filters/save", filter_set);
  ...
  async loadFilterSet(e) {
    ...
    const filter_set = await axios.post("http://localhost:5000/api/exports/filters/get", getfilterSetJson);
  ...
  render() {
    if (this.state.isLoading) return (
      ...
      <div className="w-50">
          ...
          <h2 class="text-center">Capture Data Tables</h2>
          <div id="accordion">
            {
              Object.entries(data).map(([table, columns]) => (
