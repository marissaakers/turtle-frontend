...
class SurveyMidreach extends Component {
  ...
  onChange = (e) => {
    ...
    /*
    if (e.target.coords != null) {

      var prevValue = valToInt( this.state.[e.target.name] );
      var diff = e.target.value - prevValue;
      // Find the column and add the diff to it. Maybe use something called 'coords'?
      rowSum += diff;
      colSum += diff;
    }
    */
      ...
      tableRows.push (
        <>
          <tr key={key++}>
            ...
            <td key={key++}> <input className="form-control" type="number" name={ inputNamesArr[n++] } /> </td>
            <td key={key++}> <input className="form-control" type="number" name={ inputNamesArr[n++] } /> </td>
            <td key={key++} className="bold-right-border"> <input className="form-control" type="number" name={ inputNamesArr[n++] } /> </td>
